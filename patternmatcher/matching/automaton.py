# -*- coding: utf-8 -*-
"""Contains the :class:`Automaton` which can be used for fast many-to-one matching.

You can initialize the automaton with a list of the patterns that you wish to match:

>>> pattern1 = f(a, x_)
>>> pattern2 = f(y_, b)
>>> automaton = Automaton(pattern1, pattern2)

You can also add patterns later:

>>> pattern3 = f(a, b)
>>> _ = automaton.add(pattern3)

Then you can match a subject against all the patterns at once:

>>> subject = f(a, b)
>>> for matched_pattern, substitution in sorted(automaton.match(subject)):
...
class Automaton:
    ...
    def __init__(self, *patterns: Expression) -> None:
        """
        Args:
            *patterns: The patterns which the automaton should match.
        """
        self.patterns = []
        self.states = []
    ...
    def add(self, pattern: Expression) -> int:
        """Add a new pattern to the automaton.

        Equivalent patterns are not added again. However, patterns that are structurally equivalent,
        but have different constraints or different variable names are distinguished by the automaton.
    ...
    def match(self, subject: Expression) -> Iterator[Tuple[Expression, Substitution]]:
        """Match the subject against all the automaton's patterns.
